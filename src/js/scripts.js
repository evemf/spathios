$(function() {
    const options = {
        startDay: 1,
        customDays: ['Lun', 'Mar', 'Mie', 'Jue', 'Vie', 'Sab', 'Dom'],
        customMonths: ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre']
    }

    pickerA = datepicker('#datepickerFrom', options);
    pickerB = datepicker('#datepickerTo', options);

    $(".listing-item__image").css('background', function () {
        var bg = ('url(' + $(this).data("img-src") + ') no-repeat center center fixed');
        return bg;
    });
});
